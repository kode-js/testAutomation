...
test('Popular banks validation', async ({ page }) => {
  ...
  await test.step('Top navigation elements are visible', async () => {
  ...
  await test.step('Loading spinner hidden before interacting with accounts', async () => {
  ...
  await test.step('Accounts canvas shows expected elements', async () => {
    await test.step('Verify accounts canvas elements visibility and attributes', async () => {
  ...
  for (const bank of bankslist) {
    ...
    await test.step(`Verify and open popular bank ${bank.bank}`, async () => {
    ...
    await page.waitForLoadState('networkidle');
    await test.step('Accounts offcanvas is visible after cancel', async () => {
      await expect.soft(accountsCanvas.accontsOffcanvasTitle.locator).toBeVisible();
    });
  }
