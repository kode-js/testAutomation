...
test.only('Merchant Dashboard Validation', async ({ page }) => {
  ...
  await test.step('Verify Mouse hover on Net Sales chart', async () => {
    ...
    await merchantPage.chart_NetSales.locator.hover();

    //wait for 1 second
    await sleep(1000);

    const tickValue = await merchantPage.netsalesSelectedChartValue.locator.textContent();
    ...
    await test.step('Net sales label should display hovered chart date', async () => {
      await expect.soft(merchantPage.netSalesLabel.locator).toBeVisible();
      const netSalesLabelText = await merchantPage.netSalesLabel.locator.textContent();
    ...
    await test.step('Previous year net sales label should display hovered chart date', async () => {
      await expect.soft(merchantPage.previousYearNetSalesLabel.locator).toBeVisible();
      const previousYearNetSalesLabelText = await merchantPage.previousYearNetSalesLabel.locator.textContent(); 
    ...
    while (attempts < 5) {
      await merchantPage.chart_NetSales.locator.hover({ position: { x: 10, y: 100 } });
      await sleep(1000);
      const newTickValue = await merchantPage.netsalesSelectedChartValue.locator.textContent();
