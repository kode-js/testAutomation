...
test.describe('Test case 1', () => {
  ...
  test('Verify home dashboard', async ({ page }) => {
    ...
    await expect.soft(homePage.accontsOffcanvasTitle).toBeVisible();
    await expect.soft(homePage.accountsCanvasCloseButton).toBeVisible();
    await expect.soft(homePage.bankingTab).toBeVisible();
    await expect.soft(homePage.appsTab).toBeVisible();
    await expect.soft(homePage.bankingTab).toHaveAttribute('aria-selected', 'true');
    await expect.soft(homePage.searchMoreBanksTxt).toBeVisible();
    await expect.soft(homePage.popularBanksLabel).toBeVisible();
    const bankNames = await homePage.listOfBanks.allTextContents()
    ...
    for (const bank of bankslist) {
      const bankButton = homePage.getBankByName(bank.bank);
      await expect.soft(bankButton).toBeVisible();
      await bankButton.click();
      await page.waitForLoadState('networkidle');
      await expect.soft(homePage.bankHeaderTitle).toHaveText(bank.bank);
      ...
      await homePage.canvasCancelButton.click();
      await page.waitForLoadState('networkidle');
      await expect.soft(homePage.accontsOffcanvasTitle).toBeVisible();
    }
